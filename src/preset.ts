export const checkStartup = (requiredEnvVars: string[]): void => {
  ...
  if (nonConfiguredVariables.length) {
    console.log(
      `[\x1b[31mERROR\x1b[37m] - [startup] - The next required environment variables are not configured: ${nonConfiguredVariables.join(
        ', ',
      )}`,
    );
    process.exit(1);
